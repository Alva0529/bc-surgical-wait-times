-- 20_reconciliation_quarterly.sql
--
-- Gold: what suppression costs, at quarterly grain.
--
--     python src/run_sql.py sql/gold/20_reconciliation_quarterly.sql data/warehouse.duckdb
--
-- One row per check: a published total, the children it is a total of, and what
-- can be said about the difference. A check is an edge of the hierarchy, a
-- period, a parent row and one measure.
--
-- Three edges, because the hierarchy has two dimensions and both of them have
-- parents and children:
--
--   procedures within a level   'All Procedures' over its 84 categories, at the
--                               province, at a health authority, at a facility
--   province from authorities   the province row over the six authority rows,
--                               for the same procedure group
--   authority from facilities   an authority row over its facilities, for the
--                               same procedure group
--
-- What each row says:
--
--   published        the figure on the parent row
--   reported         the children that were published, added up
--   gap              published - reported: what the children do not show
--   lower_bound      the children's minimums added up: a suppressed count is
--   upper_bound      at least 1 and at most 4, so the parent should land here
--   within_bounds    whether it does
--   implied_average  gap / suppressed children: where in the 1 to 4 band the
--                    hidden values actually sit, on average
--
-- The gap is not assumed to be suppression. It is compared against what
-- suppression could account for, and the rows where that fails are the point of
-- the whole exercise: a gap outside the bounds has some other cause — a missing
-- child row, a restated figure, a misread hierarchy — and is reported rather
-- than absorbed. docs/not-provided.md records that nothing here estimates a
-- withheld value; the answer is an interval, not a number.
--
-- This view is the one table in the model that deliberately mixes levels: the
-- province check and the facility checks sit in the same result. Summing it
...
children_rows AS (
    ...
    SELECT
        ...
        fiscal_year, quarter, 'All Health Authorities', 'All Facilities',
        procedure_group,
        completed, completed_state, completed_min, completed_max,
        waiting, waiting_state, waiting_min, waiting_max
    FROM silver_quarterly
    ...
    SELECT
        ...
        fiscal_year, quarter, health_authority, 'All Facilities',
        procedure_group,
        completed, completed_state, completed_min, completed_max,
        waiting, waiting_state, waiting_min, waiting_max
    FROM silver_quarterly
...
children AS (
    ...
    FROM (
        SELECT
            children_rows.edge,
            children_rows.fiscal_year, children_rows.quarter,
    ...
    GROUP BY edge, fiscal_year, quarter, health_authority, hospital_name,
...
published AS (
    SELECT
        parents.edge,
        parents.fiscal_year, parents.quarter,
...
LEFT JOIN children
    ...
    AND children.quarter          = published.quarter
...
SELECT
    edge, fiscal_year, quarter, health_authority, hospital_name, procedure_group,
